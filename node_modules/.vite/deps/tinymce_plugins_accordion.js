...
var require_plugin = __commonJS({
  "node_modules/tinymce/plugins/accordion/plugin.js"() {
    (function() {
      ...
      const scan = (universe2, element, direction) => {
        if (universe2.property().isText(element) && universe2.property().getText(element).trim().length === 0 || universe2.property().isComment(element)) {
          return direction(element).bind((elem) => {
            return scan(universe2, elem, direction).orThunk(() => {
      ...
      const freefallRtl$2 = (universe2, element) => {
        const candidate = scan(universe2, element, universe2.query().prevSibling).getOr(element);
      ...
      const removeAccordion = (editor) => {
        if (!editor.mode.isReadOnly()) {
          getSelectedDetails(editor).each((details) => {
            const { nextSibling: nextSibling2 } = details;
            if (nextSibling2) {
              editor.selection.select(nextSibling2, true);
      ...
      const addClasses = (node, classes) => {
        const classListSet = /* @__PURE__ */ new Set([
          ...getClassList(node),
          ...classes
        ]);
      ...
      const getAccordionChildren = (accordionNode) => {
        const children2 = accordionNode.children();
        let summaryNode;
        let wrapperNode;
        const otherNodes = [];
        for (let i = 0; i < children2.length; i++) {
          const child2 = children2[i];
          if (child2.name === "summary" && isNullable(summaryNode)) {
            summaryNode = child2;
          } else if (isAccordionBodyWrapperNode(child2) && isNullable(wrapperNode)) {
            wrapperNode = child2;
          } else {
            otherNodes.push(child2);
          }
        }
        return {
          summaryNode,
      ...
      const setup$2 = (editor) => {
        editor.on("PreInit", () => {
          ...
          serializer.addNodeFilter(accordionTag, (nodes) => {
            const summaryClassRemoveSet = /* @__PURE__ */ new Set([accordionSummaryClass]);
            for (let i = 0; i < nodes.length; i++) {
              const node = nodes[i];
              if (isAccordionDetailsNode(node)) {
                const accordionNode = node;
...
// node_modules/tinymce/plugins/accordion/index.js
require_plugin();
//# sourceMappingURL=tinymce_plugins_accordion.js.map
