  ...
  <div class="knowledge-container">
    ...
    <div class="header"> 
      ...
      <div class="search-section">
        ...
        <el-icon class="refresh-icon" @click="refreshList"><Refresh /></el-icon>
      </div>
    </div>
...
const navigateToTopicUpload = (topic) => {
  // 通知父组件切换到上传组件并传递知识库信息
  console.log('导航到上传组件--------->:', topic)
  
  // emit('to_topic_upload', topic)
  const customEvent = new CustomEvent('navigate-to-upload', {
    detail: { 
      // classifyId: topic.id, 
      // classifyName: topic.classifyName, 
      // classifyDesc: topic.classifyDesc,
      topic: topic  // 添加完整的 topic 对象
    }
  })
  console.log('导航到上传组件--------->customEvent:', customEvent)
  window.dispatchEvent(customEvent)

   // 使用 mitt 发送事件，替代 CustomEvent
