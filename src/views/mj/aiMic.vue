<script setup>
import { mlog } from '@/api/mj';
import SvgIconTwo from '@/components/SvgIconTwo';
...
<template v-if="!st.isGo">
    <NButton @click="start()" type="primary" block round> 
        <template #icon><SvgIconTwo icon="bi:mic"/></template>{{ $t('mj.mStart') }}
    </NButton>
</template>
<n-button-group v-else>
    ...
    <NButton type="primary" @click="pause()" v-if="st.start==1">
        <template #icon><SvgIconTwo icon="bi:pause-circle"/></template>{{ $t('mj.mPause') }}
    </NButton>
    <NButton type="primary" @click="pauseGoon()" v-if="st.start==2">
        <template #icon><SvgIconTwo icon="bi:mic"/></template>{{ $t('mj.mGoon') }}
    </NButton>
 
    <template v-if="stat.duration>0 ">
        <NButton type="info" @click="stopAdnRecord()"  v-if="!isMobile" >
        <template #icon><SvgIconTwo icon="bi:bootstrap-reboot"/></template>{{ $t('mj.mRecord') }}</NButton>

        <NButton @click="play()" type="info">
        <template #icon><SvgIconTwo icon="bi:play-circle"/></template>{{ $t('mj.mPlay') }}</NButton> 
        <NButton @click="cancal()" type="info">
        <template #icon><SvgIconTwo icon="ri:close-circle-line"/></template>{{ $t('mj.mCanel') }}</NButton> 

        <NButton type="primary" @click="send()">
        <template #icon><SvgIconTwo icon="ri:send-plane-fill"></SvgIconTwo></template>
        {{ $t('mj.mSent') }}
