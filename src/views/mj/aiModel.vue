...
import { listKnowledge } from '@/api/glxt/knowledge'
...
const fetchDataGetKnowledge = async () => {
	if(getToken()){
		try {
			// 发起一个请求
			const [err, result] = await to(listKnowledge());
			console.log("result===", result.rows)
			if (err) {
				ms.error(err.message)
			} else {
				options.value = result.rows.map((item) => ({
...
const modellist = computed(() => { //
	...
	// 	let delModel = [];
	// 	let addModel =[];
	// 	homeStore.myData.session.cmodels.split(/[ ,]+/ig).map( (v)=>{
	// 		if(v.indexOf('-')==0){
...
const onSelectChange1 = (newValue) => {
